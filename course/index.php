...
<head>
    ...
    <title>Questions <?php echo $title; ?></title>
</head>
<body>
    ...
    <div class="course">
        ...
        <h1 class="title_orange title_fz48 course__subject-title">Subject: <?php echo $course["subject"] ?></h1>
            ...
            <?php 
                $select = "SELECT `name` FROM `topic` WHERE course = '$courseName'";
                $query = mysqli_query($db,$select);
                $topics = mysqli_fetch_all($query, MYSQLI_ASSOC);//запрос и аргумент для ассоциативного показа
                $i = 1;
                foreach ($topics as $topic) { ?>
                        <div class="course__topics-item">
                            <h4 class="course__item-title"><span><?php echo $i; $i++; ?></span> <?php  echo $topic['name']; ?></h4>
                </div><?php } ?>
                </div>
